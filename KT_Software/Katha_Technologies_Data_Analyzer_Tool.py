...
class CorporateReportBuilder:
    ...
    def _build_ui(self):
        # ================= Header =================
        header_frame = tk.Frame(self.root, bg="#f5f5f5", height=80)
        header_frame.pack(fill="x")

        # Logo
        try:
            logo_img = Image.open("company_logo.png")
            logo_img = logo_img.resize((160, 55), Image.LANCZOS)
            self.logo_img = ImageTk.PhotoImage(logo_img)  # KEEP reference

            logo_lbl = tk.Label(header_frame, image=self.logo_img, bg="#f5f5f5")
            logo_lbl.pack(side="left", padx=15, pady=10)
        except Exception:
            pass  # app still runs if logo missing

        title_lbl = tk.Label(
            header_frame,
            text="Corporate Data Analyzer (Confidential & Internal Use Only)",
            font=("Arial", 12, "bold"),
            bg="#f5f5f5",
            fg="gray"
        )
        ...
        # -------- Info --------
        info_frame = tk.LabelFrame(content_frame, text="File Info", padx=10, pady=8)
        ...
        controls = tk.LabelFrame(
            content_frame,
            text="Build Report (GroupBy + Aggregation)",
            padx=10,
            pady=10
        )
        ...
        # -------- Chart Builder --------
        chart_frame = tk.LabelFrame(content_frame, text="Chart Builder", padx=10, pady=10)
